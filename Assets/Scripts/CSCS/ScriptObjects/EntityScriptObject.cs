...
namespace CSCS
{
    public class EntityScriptObject : ScriptObject
    {
        ...
        private UnityEntity m_EntityObject = null;
        private Archetype m_ObjectArchetype = Archetype.Empty;
        ...
        public EntityScriptObject(GameObject unityEntityPrefab)
        {
            var mre = new ManualResetEvent(false);
            CscsScriptingController.ExecuteInUpdate(() =>
            {
                m_EntityObject = GameObject.Instantiate(unityEntityPrefab).GetComponent<UnityEntity>();
                mre.Set();
            });
            ...
            CscsScriptingController.ExecuteInUpdate(() =>
            {
                switch (sPropertyName)
                {
                    ...
                    case "Scale":
                        ...
                        break;
                    case "Archetype":
                        newValue = new Variable(GetNameOfArchetype());
                        break;
            ...
            CscsScriptingController.ExecuteInUpdate(() =>
            {
                switch (sPropertyName)
                {
                    ...
                    case "RotateAround":
                        ...
                        break;
                    case "Archetype":
                        m_ObjectArchetype = GetArchetypeFromString(argValue.AsString());
                        break;
        ...
        {
            var myVector3 = m_EntityObject.transform.position;
            return GetVariableFromVector3(myVector3);
        }

        public Variable SetPositionProperty(Vector3 aVector3)
        {
            m_EntityObject.transform.position = aVector3;
            return Variable.EmptyInstance;
        }
        
        public Variable GetScaleProperty()
        {
            var myVector3 = m_EntityObject.transform.localScale;
            return GetVariableFromVector3(myVector3);
        }

        public Variable SetScaleProperty(Vector3 aVector3)
        {
            m_EntityObject.transform.localScale = aVector3;
            return Variable.EmptyInstance;
        }
        
        public Variable GetRotationProperty()
        {
            Quaternion quaternion = m_EntityObject.transform.rotation;
            return GetVariableFromQuaternion(quaternion);
        }

        public Variable SetRotationProperty(Quaternion aQuaternion)
        {
            m_EntityObject.transform.rotation = aQuaternion;
            return Variable.EmptyInstance;
        }
        
        public Variable DebugLog(Variable args)
        {
            Debug.Log(args.AsString());
            return Variable.EmptyInstance;
        }
        ...
        public Variable Translate(Variable vectorVariable)
        {
            ...
            m_EntityObject.transform.Translate(aVector3);
            return Variable.EmptyInstance;
        }
        ...
        {
            var aVector3 = GetVector3FromVariable(vectorVariable);
            m_EntityObject.transform.RotateAround(aVector3,vectorVariable.GetValue(3).AsFloat());
            return Variable.EmptyInstance;
        }

        private string GetNameOfArchetype()
        {
            switch (m_ObjectArchetype)
            {
                case Archetype.Empty:
                    return "Empty";
                case Archetype.StaticMesh:
                    return "StaticMesh";
                case Archetype.AnimatedMesh:
                    return "AnimatedMesh";
                default:
        ...
        private Archetype GetArchetypeFromString(string archetype)
        {
            switch (archetype)
            {
                case "Empty":
                ...
                case "StaticMesh":
                    return Archetype.StaticMesh;
                case "AnimatedMesh":
                    return Archetype.AnimatedMesh;
                default:
                    return Archetype.Empty;
            }
        }
